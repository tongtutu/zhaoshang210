...
final class SnowFlake
{
    ...
    const TWEPOC = 1534815701000; // 2018-08-21
    ...
    public function nextId()
    {
        ...
        return PHP_INT_SIZE == 4 ? $this->getTimestamp() . '-' . Utils::randStr(5, 4) : $this->mintId64($time, $datacenter, $worker, $sequence);
    }
    ...
    private function setDatacenterId($datacenterId)
    {
        if ($datacenterId > $this->maxDatacenterId() || $datacenterId < 0) {
            throw new \Exception("invalid argument datacenterId");
        }
    ...
    private function setWorkerId($workerId)
    {
        if ($workerId > $this->maxWorkerId() || $workerId < 0) {
            throw new \Exception("invalid argument workerId");
        }
